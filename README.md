...
The optional `research_context.enabled` observer records SPX IV/term structure and the economic calendar. It runs asynchronously after a completed trading cycle, from 08:00 ET to 15 minutes after the configured session close on open trading days. IV and term structure refresh at most every 30 minutes; the calendar every six hours. One request is in flight at a time. Errors, materialization misses and HTTP 429 back off without waiting in the trading loop. All three sources remain background only: daily IV is explicitly not realtime IV, missing values stay null, and an empty provider calendar is not proof of no event risk. The observer does not change entries, sizing, exits, or VEX rules. Its state and recorder health appear in `/api/status` under `junk_status.research_context`.

Use `npm run research:snapshot` for a read-only provider check (requires the existing `YEHANGSHE_API_KEY` environment variable). It writes `logs/zero-dte-options-research-context.json` and appends the timestamped source observations to `logs/zero-dte-options-research-context.ndjson`. The watcher also attaches context to decision audits only after all strategy and execution gates have run.

Use `npm run research:replay -- --date 2026-09-16` (or omit `--date` for all recorded days) to produce a local HTML/JSON review under `logs/research-reports/`. The report separates each US trading day and retained exit line, uses complete comparable actual fills and invested-cost returns, honors the void-day registry, and links each cohort to Nightwatch Replay Lab. Beginning with this deployment, the bounded, asynchronous `replay-observations.ndjson` journal records the actual option quote and underlying inputs already obtained for exit management. Offline replay reuses the production exit planner and each cohort's frozen rules, assuming immediate fills at bid minus the configured buffer. Missing/stale quotes, gaps over 45 seconds, or paths ending before the modeled exit are excluded; no quote midpoint, intrabar path, queue fill or profit is fabricated. Historical trades without a complete quote journal show actual fills and an explicit replay coverage gap. Neither research command submits orders or writes the trading ledger. Research files remain local and ignored by Git.
