...
- `target_pct` 表示这 5 个标的之间的相对权重。总投入比例默认 `100%`；如果要保留约 `15%` 现金，使用 `--target-invested-pct 85` 或设置 `STOCK_REBALANCE_TARGET_INVESTED_PCT=85`。
- 只按整股配平，不买碎股。
...
npm run stock:rebalance-extended
```
