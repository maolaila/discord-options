...
function loadState() {
  for (const candidate of [statePath, ...legacyStatePaths]) {
    ...
    try {
      ...
      throw new Error(`PA exit state is corrupt and cannot be recovered: ${candidate}: ${primaryError.message}`);
    }
  }
  return { orders: {} };
}
...
async function writeState(state) {
  ...
  if (fs.existsSync(statePath)) await fsp.copyFile(statePath, `${statePath}.bak`);
...
function submittedBuyPlans(mode) {
  ...
  for (const logPath of [executionsPath, ...legacyExecutionsPaths]) {
    for (const plan of readNdjson(logPath)) {
      ...
      if (plan.business_line && plan.business_line !== businessLine.key) continue;
      if (/^SPXW?\d{6}[CP]/i.test(String(plan.order?.code || ''))) continue;
      const isLegacyPath = legacyExecutionsPaths.includes(logPath);
      if (isLegacyPath && !plan.business_line && !String(plan.order?.remark || '').startsWith('discord:')) continue;
      const ids = brokerOrderIds(plan.execution?.response?.s2c || {});
      const orderKey = ids.order_id_ex || ids.order_id;
      if (!orderKey) continue;
      seen.set(orderKey, {
        ...plan,
        business_line: plan.business_line || businessLine.key,
        broker_buy_order_ids: ids,
      });
    }
  }
...
async function processOnce(client, config, state, quoteFeed, mode) {
  ...
  const activePaPosition = plans.some(({ plan }) => positionList.some((position) => (
    String(position?.code || '') === String(plan.order?.code || '') && (numeric(position?.qty) || 0) > 0
  )));
  const unresolvedExitSubmission = Object.values(state.orders || {}).some((row) => (
    ['exit_intent', 'exit_submission_unknown', 'exit_submitted', 'exit_waiting_fill'].includes(String(row?.status || ''))
  ));
  const brokerUrgent = activePaPosition || unresolvedExitSubmission;
  let orderSnapshot = null;
  try {
    orderSnapshot = await getSharedPaOrderSnapshot({
      snapshotPath: sharedOrderSnapshotPath,
      lockPath: sharedOrderSnapshotLockPath,
      // A live PA position must see broker order state inside 15 seconds. With
      // the account-wide 11-second permit this 8-second cache age makes the next
      // five-second exit cycle wait for and consume the permit at about t+11s.
      maxAgeMs: brokerUrgent ? 8_000 : 30_000,
      waitForPermit: brokerUrgent,
      maxWaitMs: 15_000,
      fetchOrders: async () => normalizeForJson((await fetchOrderList(client, config)).s2c?.orderList || []),
    });
  } catch (error) {
    // Fail closed and keep the current exit state. A broker read outage must
    // never be interpreted as proof that a prior sell was not accepted.
    orderSnapshot = {
      ok: false,
      degraded: true,
      source: 'snapshot_exception',
      orders: [],
      reason: 'shared_order_snapshot_exception',
      error: error.message,
    };
  }
  if (!orderSnapshot.ok) {
    await writeStatus({
      ...
      mode,
      plans: plans.length,
      active_pa_position: activePaPosition,
      unresolved_exit_submission: unresolvedExitSubmission,
      watched: 0,
      submitted_exits: 0,
    ...
    return {
      ...
      submittedExits: 0,
      reconciledExits: 0,
      unresolvedExits: unresolvedExitSubmission ? 1 : 0,
      degraded: true,
    };
  }
  const orderList = orderSnapshot.orders;
  ...
  const knownExitRemarks = new Set(Object.values(state.orders || {})
    .map((row) => String(row?.exit_remark || ''))
    .filter(Boolean));
  const orphanActiveExitOrders = orderList.filter((order) => String(order?.remark || '').startsWith(PA_EXIT_REMARK_PREFIX)
    && Number(order?.trdSide) === 2
    && isPotentiallyActiveBrokerOrder(order)
    && !knownExitRemarks.has(String(order?.remark || '')));
  let submittedExits = 0;
  let watched = 0;
  let reconciledExits = 0;
  let unresolvedExits = 0;
  let controlledOvernightHeld = activeControlledOvernightCount(state);

  for (const { orderIDEx, plan } of plans) {
    let stateRow = state.orders[orderIDEx] || {};
    if (stateRow.status === 'closed') continue;
    const orphanForCode = orphanActiveExitOrders.filter((order) => String(order?.code || '') === String(plan.order?.code || ''));
    if (orphanForCode.length > 0) {
      unresolvedExits += 1;
      stateRow = {
        ...stateRow,
        status: 'exit_orphan_broker_order_fail_closed',
        exit_reconciliation: {
          resolution: 'unresolved',
          reason: 'active_pa_exit_without_local_state',
          match_count: orphanForCode.length,
          broker_order_ids: orphanForCode.map((order) => brokerOrderIds(order)),
          checked_at: new Date().toISOString(),
        },
        updated_at: new Date().toISOString(),
      };
      state.orders[orderIDEx] = stateRow;
      await writeState(state);
      continue;
    }
    if (stateRow.status === 'exit_intent' || stateRow.status === 'exit_submission_unknown') {
      const recovery = reconcilePaExitAttempt({
        stateRow,
        brokerOrders: orderList,
        code: plan.order?.code,
      });
      if (recovery.resolution === 'unresolved') {
        unresolvedExits += 1;
        stateRow = {
          ...stateRow,
          exit_reconciliation: {
            ...
            match_count: recovery.match_count ?? null,
            checked_at: new Date().toISOString(),
          },
        };
        state.orders[orderIDEx] = stateRow;
      ...
      stateRow = {
        ...
        exit_reconciliation: {
          resolution: recovery.resolution,
          checked_at: new Date().toISOString(),
          broker_order_ids: brokerOrderIds(recovery.broker_order || {}),
        },
      };
      state.orders[orderIDEx] = stateRow;
      await writeState(state);
    ...
    if (stateRow.status === 'exit_submitted' || stateRow.status === 'exit_waiting_fill') {
      const exitOrder = findBrokerOrderByIds(ordersById, {
        order_id: stateRow.exit_order_id,
    ...
    const exitAttempt = Math.max(0, Math.floor(numeric(stateRow.exit_attempt) || 0)) + 1;
    const remark = buildPaExitRemark({
      executionKey: plan.execution_key || plan.signal?.source_signal_key || '',
      buyOrderKey: orderIDEx,
      attempt: exitAttempt,
    });
    nextState = {
      ...nextState,
    ...
    try {
      ...
      exitOrderIds = brokerOrderIds(normalizeForJson(response)?.s2c || {});
      if (!(exitOrderIds.order_id_ex || exitOrderIds.order_id)) {
        throw new Error('PlaceOrder exit returned success without orderID or orderIDEx; submission outcome is unknown.');
      }
    } catch (error) {
      nextState = {
        ...nextState,
      ...
      await appendTradeJournalEvent(
        ...
        buildExitJournalPayload({
          ...
          config,
          sourceBuyOrderIDEx: orderIDEx,
          lifecycleStatus: 'exit_submission_unknown',
          brokerOrder: normalizeForJson(order || null),
          fills: normalizeForJson(fills || null),
          position: normalizeForJson(position || null),
          filledQty,
    ...
    nextState = {
      ...
      exit_order_id: exitOrderIds.order_id || null,
      exit_order_id_ex: exitOrderIds.order_id_ex || null,
  ...
  await writeStatus({
    ...
    reconciled_exits: reconciledExits,
    unresolved_exits: unresolvedExits,
    orphan_active_exit_orders: orphanActiveExitOrders.map((order) => ({
      ...brokerOrderIds(order),
  ...
  return { plans: plans.length, watched, submittedExits, reconciledExits, unresolvedExits };
}
