...
function parseNightwatchFlowSignal(record, observedVia) {
  ...
  const signal = {
    ...
    author_bot: Boolean(record.author && record.author.bot),
    advice_format: 'flow',
...
function buildOrderIntent(signal) {
  // Archival/REST signals are never executable. Capture must explicitly mark a
  // freshly observed, authenticated Gateway MESSAGE_CREATE as live eligible.
  if (!signal.signal_actionable || signal.live_eligible !== true) return null;
  ...
  return {
    ...
    author_bot: signal.author_bot === true,
    captured_at: signal.captured_at,
    capture_lag_ms: signal.capture_lag_ms,
    live_eligible: true,
    action: 'BUY_TO_OPEN',
