...
var mmWebsocketSection = 1;
const mmWebsocketHeadLength = 44; //包头大小
const mmWebsocketHeadSign = "ft-v1.0";


/**
 * @name mmWebsocketBase
 * @class websocket封装工具类
 * @constructor mmWebsocketBase
...
class mmWebsocketBase {
    ...
    packBuff(cmd, section, buff) {
        ...
        pbuff.writeUTF8String(mmWebsocketHeadSign);
        let completeLength = 8 - bytebuffer.calculateString(mmWebsocketHeadSign);
        if (completeLength > 0) {
    ...
    unpackBuff(buff) {
        if (buff instanceof ArrayBuffer) {
            ...
            if (buff.byteLength > mmWebsocketHeadLength) {
                let data = pbuff.readBytes(buff.byteLength - mmWebsocketHeadLength);
    ...
    sendBuff(cmd, buff, timeout) {
        ...
        if (this.websock && this.websock.readyState == WebSocket.OPEN) {
            if (this.state.login || cmd == mmApiCmdID.Init) //没有初始化之前不允许请求
            {
                section = ++mmWebsocketSection;
    ...
    recvBuff(buff) {
        ...
        if (result !== null && result.sign !== null && result.sign !== undefined) {
            if (result.sign.indexOf(mmWebsocketHeadSign) == -1) {
                console.log(result.sign, "||", mmWebsocketHeadSign);
                error = mmWebsocketRecvError.ErrorSign;
    ...
    */
    ...
    initWebSocket(configBuffer) {
        ...
        this.websock.onopen = () => {
            //连接建立之后执行send方法发送数据
            this.sendBuff(mmApiCmdID.Init, this.initOpenDConfigBuff, 20000)
                .then(response => {
...
export default mmWebsocketBase
